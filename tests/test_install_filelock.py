...
# ---------------------------------------------------------------------------
# notify_pull integration test — the actual silent-rollback bug. The
# supervisor's pull-driven restart calls _run_install then restart_app
# unconditionally; on install failure we must skip the restart instead.
# ---------------------------------------------------------------------------

@pytest.mark.asyncio
async def test_notify_pull_skips_restart_on_install_failure(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch,
):
    ...
    # Avoid hitting the real Windows-only app_store path.
    monkeypatch.setattr(app_manager.app_store, "set_last_sha", lambda *a: None)
    monkeypatch.setattr(app_manager.app_store, "get", lambda name: rt.record)

    restarted: list[str] = []
    async def fake_restart(name: str) -> bool:
        restarted.append(name)
        return True
    monkeypatch.setattr(mgr, "restart_app", fake_restart)

    terminated: list[str] = []
    async def fake_terminate(name: str) -> None:
        terminated.append(name)
    monkeypatch.setattr(mgr, "_terminate", fake_terminate)

    cycled = await mgr.notify_pull("repo", new_sha="a" * 40)
    ...
    assert rt.install_status == "failed"
    # We still report the app as 'cycled' so the operator sees it
    # acknowledged the pull — but its install_status flags the failure.
    assert "jeeves" in cycled
    # The defensive terminate ran to ensure the previous PID is gone.
    assert terminated == ["jeeves"]


@pytest.mark.asyncio
async def test_notify_pull_restarts_on_install_success(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch,
):
    monkeypatch.setattr(app_manager, "LOG_DIR", tmp_path / "logs")
    (tmp_path / "logs").mkdir()
    ...
    cycled = await mgr.notify_pull("repo", new_sha="b" * 40)

    assert restarted == ["jeeves"]
    assert rt.install_status == "ok"
    assert cycled == ["jeeves"]
